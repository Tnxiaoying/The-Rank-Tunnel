...
# --- 实验 4: 压力测试 (Blow-up Test) ---
# 寻找奇异点：在极端条件下观察流速发散
def plot_blowup_test():
    print("Generating Figure 4: Hunting the Singularity (Blow-up Test)...")
    
    # 设定极端条件：超高难度 + 极低粘性 (高噪声)
    # 这模拟了模型在面对超出能力范围的问题时的“幻觉风暴”
    sim = FluidRankTunnel(dim=64, target_rank=64, viscosity=0.1) 
    sim.run(steps=100)
    
    velocities = np.array(sim.velocities)
    
    plt.figure(figsize=(10, 6))
    plt.plot(velocities, 'r-', linewidth=2, label='Velocity ($u_t$)')
    
    # 寻找峰值，模拟爆破点
    if len(velocities) > 0:
        max_v_idx = np.argmax(velocities)
        max_v = velocities[max_v_idx]
        plt.plot(max_v_idx, max_v, 'ko', markersize=8)
        plt.text(max_v_idx + 2, max_v, 'Finite-time Singularity?\n(Hallucination Peak)', color='k', fontweight='bold')
    
    plt.xlabel('Inference Steps')
    plt.ylabel('Flow Velocity magnitude ($|u_t|$)')
    plt.title('Blow-up Test: Hunting for Finite-time Singularities')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('blowup_test.png')
    plt.close()
